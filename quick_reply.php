...
$template->block('quick_reply', array(
...
{
	$template->block('quick_reply.user_logged_in', array(
...
if ( $is_auth['auth_mod'] )
{
	...
	{
		$template->block('quick_reply.switch_unlock_topic', array());
		$template->vars(array(
	...
	{
		$template->block('quick_reply.switch_lock_topic', array());
		$template->vars(array(
...
{
	$template->block('quick_reply.expire_box', array());
	$template->vars(array(
...
if ( $board_config['split_messages'] )
{
	...
	{
		$template->block('quick_reply.switch_no_split_post', array());
...
if (!( !$board_config['allow_bbcode'] || (!$userdata['session_logged_in'] && $board_config['allow_bbcode_quest']) ))
{
	...
	{
		$template->block('quick_reply.button_b', array());
	}
	if ( $board_config['button_i'] )
	{
		$template->block('quick_reply.button_i', array());
	}
	if ( $board_config['button_u'] )
	{
		$template->block('quick_reply.button_u', array());
	}
	if ( $board_config['button_im'] )
	{
		$template->block('quick_reply.button_im', array());
	}
	if ( $board_config['button_c'] )
	{
		$template->block('quick_reply.button_c', array());
	}
	if ( $board_config['button_q'] )
	{
		$template->block('quick_reply.button_q', array());
	}
}

if ( $board_config['allow_bbcode'] == 1 )
{
	$template->block('quick_reply.quote_box', array());
}
...
$template->var('QUICKREPLY_OUTPUT', $template->render('quick_reply.tpl'));
